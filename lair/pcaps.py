...
def determine_pcap_events(
    ...
    pcap_events = pd.DataFrame({"start": times.min(), "end": times.max()}).reset_index(
        drop=True
    )
...
def filter_pcap_events(
    data: pd.DataFrame, events: pd.DataFrame, level=None
) -> pd.DataFrame:
    """
